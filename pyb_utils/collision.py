...
class CollisionDetector:
    ...
    def get_link_object_distance(self, object_name, object_link=-1, max_distance=1.0, ignore_link=None):
        # Compute distances between all links and a specific obstacle
        distances = []
        for link in self.robot_link_ids.keys():
            if link in ignore_link:
                continue
            closest_points = p.getClosestPoints(
                self.robot_id,
                self.bodies[object_name],
                distance=max_distance,
                linkIndexA=self.robot_link_ids[link],
                linkIndexB=object_link,
                physicsClientId=self.col_id,
            )
    ...
    def compute_distances_per_collision_shape(self, max_distance=1.0):
        """Compute closest distances configuration sorted by link of robotic arm.
        ...
        info = {}
        c_points = {}
        for link, obs in self.indexed_collision_pairs:
            # iterate over each collision shape
            collision_shapes = p.getCollisionShapeData(self.robot_id, link.body_uid)
            for shape in collision_shapes:
                collision_shape_index = shape[0]

                closest_points = p.getClosestPoints(
                    link.body_uid,
                    obs.body_uid,
                    distance=max_distance,
                    linkIndexA=link.link_uid,
                    linkIndexB=obs.link_uid,
                    physicsClientId=self.col_id,
                )
                ...
                # if link doesn't have a dict entry, add
                if distances.get(link.link_uid) is None:
                    distances[link.link_uid] = []
                    c_points[link.link_uid] = []

                # if bodies are above max_distance apart, nothing is returned, so
                # we just saturate at max_distance. Otherwise, take the minimum
                if len(closest_points) == 0:
                    distances[link.link_uid].append(max_distance)
                else:
                    distances[link.link_uid].append(np.min([pt[8] for pt in closest_points]))
                    c = np.min([pt[8] for pt in closest_points])
                    idx = [pt[8] for pt in closest_points].index(c)

                    x = closest_points[idx][5]
                    y = closest_points[idx][6]
                    c_points[link.link_uid].append((x, y))
    ...
    def get_link_object_collision(self, shape_id, obstacle_name, max_distance=1.0):
        ...
        try:
            return result[0][8], np.array(result[0][5]), np.array(
                result[0][6])  # distance, point on robot, point on obstacle
        except ValueError:
